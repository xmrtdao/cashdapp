...
const SERVICES = [
  {
    ...
    cwd: ROOT,
    healthCheck: () => checkHttp('http://localhost:8080/health', 2000),
...
const ENV = loadEnv();
const RESEND_KEY = ENV.RESEND_API_KEY || ENV.RESEND_XMRT_API_KEY;
...
function checkProcessByScript(scriptName) {
  try {
    const out = execSync(
      `powershell.exe -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\\"Name = 'node.exe'\\\" | Where-Object { $_.CommandLine -like '*${scriptName}*' } | Select-Object -First 1 -ExpandProperty ProcessId"`,
...
function checkProcessByName(exeName) {
  // `exeName` must be the bare image name, e.g. 'postgres.exe' or 'cloudflared.exe'
  try {
    const out = execSync(
      `powershell.exe -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"Name = '${exeName}'\\" | Select-Object -First 1 -ExpandProperty ProcessId"`,
...
async function sendAlert(subject, body) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Vex Supervisor <vex@partyfavorphoto.com>',
        to: ALERT_EMAILS,
        subject,
        text: body,
      }),
